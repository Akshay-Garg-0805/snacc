      ...
      <AnimatePresence mode="wait">
        {isCustomSelected ? (
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.2 }}
            className="grid grid-cols-4 sm:grid-cols-6 gap-3 max-h-48 overflow-y-auto custom-scrollbar"
          >
            {filteredAvatars.map((avatar) => (
              <motion.button
                ...
                className={`relative w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 transition-all duration-200 flex items-center justify-center ${
                ...
                title={avatar.name}
              >
                <span className="text-2xl sm:text-3xl">{avatar.url}</span>
                {selectedAvatarId === avatar.id && !isCustomSelected && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
