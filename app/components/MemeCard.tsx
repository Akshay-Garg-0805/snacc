...
export default function MemeCard({
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Track achievements and XP for giving likes
      if (!isLiked) {
        await AchievementService.trackUserAction(user.uid, 'likes_given');

        // Award XP for giving a like
        await XPService.awardXP(user.uid, 'meme_like_given', undefined, 'Liked a meme');

        // Mark that author has ever received a like (for permanent first_like achievement)
        const authorHadNoLikes = likesCount === 0;
        if (authorHadNoLikes) {
          await UserService.markEverReceivedLike(authorId);
          await AchievementService.trackUserAction(authorId, 'first_like');
        }

        // Update author's total likes stat
        await UserService.incrementUserStats(authorId, 'totalLikes');

        // Track achievements for receiving likes (for meme author)
        await AchievementService.trackUserAction(authorId, 'total_likes_received');

        // Award XP for receiving a like (to meme author)
        await XPService.awardXP(authorId, 'meme_like_received', undefined, 'Received a like on your meme');
        ...
        try {
          ...
          // Don't break the like functionality if notification fails
        }

        // Track user interaction for liked memes history
        await UserInteractionService.addInteraction(user.uid, id, 'like', {
          memeTitle: title,
          authorId: authorId
        });

        // Check for viral meme achievements and bonus XP
        const newLikesCount = likesCount + 1;
        if (newLikesCount >= 100) {
          await AchievementService.trackUserAction(authorId, 'viral_meme', newLikesCount);
          await XPService.awardXP(authorId, 'viral_meme', undefined, 'Your meme went viral!');
        }
      } else {
        // Decrement author's total likes stat when unliked
        await UserService.incrementUserStats(authorId, 'totalLikes', -1);

        // Remove user interaction when unliked
        await UserInteractionService.removeInteraction(user.uid, id, 'like');
      }
    ...
    <>
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          ...
          <div className="flex items-center space-x-4 sm:space-x-6 mb-2">
            ...
            <motion.button
              ...
              className="flex items-center space-x-1 sm:space-x-2 text-text-secondary hover:text-blue-500 transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4 sm:h-5 sm:w-5"
      ...
      <ChatModal
        ...
        memeData={{
          id,
          title,
          imageUrl: `https://res.cloudinary.com/dvbfanque/image/upload/${publicId}`,
          authorName: authorProfile?.nickname || authorName
        }}
      />
    </>
